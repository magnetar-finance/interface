...
export const PageHeader: React.FC<PageHeaderProps> = ({
  ...
  <div className={`flex flex-col gap-2 w-full mb-4 ${className}`}>
    <div className="flex items-center gap-4 flex-wrap">
      <h1 className="text-white text-2xl font-bold uppercase tracking-widest font-sans flex items-center gap-2">
        <span
          className="font-mono text-transparent bg-clip-text bg-gradient-to-r from-[#2962ff] to-[#9d4edd] animate-shimmer"
      ...
      {chips && chips.length > 0 && (
        <div className="flex items-center gap-2 flex-wrap">
          {chips.map((chip) => (
            <span
              key={chip.label}
              className={`inline-flex items-center gap-1.5 px-2 py-0.5 border rounded-md text-[10px] font-mono uppercase tracking-widest ${
    ...
    <div className="w-full h-[2px] bg-gradient-to-r from-[#2962ff]/50 via-[#9d4edd]/20 to-transparent mt-2 rounded-full" />
  </div>
);
