...
function composeBytes(fees: Address[], tokens: Address[][], tokenId: bigint) {
  const composition: Composition = {} as Composition;
  composition.bytes = encodeFunctionData({
    abi,
    functionName: 'claimFees',
    args: [fees, tokens, tokenId],
  });

  return composition;
}

function useClaimFees(
  fees: Address[],
  tokens: Address[][],
  tokenId: bigint,
  ...
  const composition = useMemo(() => composeBytes(fees, tokens, tokenId), [fees, tokens, tokenId]);
